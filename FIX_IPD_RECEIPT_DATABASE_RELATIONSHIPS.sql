-- =======================================================
-- FIX IPD RECEIPT PRINTING - DATABASE RELATIONSHIP ISSUE
-- This script fixes the schema mismatch between patient_admissions and beds tables
...
BEGIN
    ...
    IF bed_count = 0 THEN
        ...
        ('B004', 'GENERAL', 'General Ward A', 1, 1000.00, 'AVAILABLE'),
        ('B005', 'GENERAL', 'General Ward B', 1, 1000.00, 'AVAILABLE'),
        ('P001', 'PRIVATE', 'Private Wing', 2, 2500.00, 'AVAILABLE'),
        ('P002', 'PRIVATE', 'Private Wing', 2, 2500.00, 'AVAILABLE'),
        ('P003', 'PRIVATE', 'Private Wing', 2, 2500.00, 'AVAILABLE'),
        ...
        ('E002', 'EMERGENCY', 'Emergency Ward', 1, 1500.00, 'AVAILABLE')
        ON CONFLICT (bed_number) DO NOTHING;
        
        RAISE NOTICE 'Inserted sample bed data (% beds)', bed_count;
    ELSE
...
-- =======================================================
-- STEP 4: MIGRATE EXISTING DATA FROM BED_NUMBER TO BED_ID
-- =======================================================
...
BEGIN
    ...
    LOOP
        ...
        SELECT id INTO bed_record FROM beds WHERE bed_number = admission_record.bed_number LIMIT 1;
        
        IF bed_record.id IS NOT NULL THEN
            -- Update with existing bed
            UPDATE patient_admissions 
            SET bed_id = bed_record.id 
            WHERE id = admission_record.id;
            updated_count := updated_count + 1;
        ELSE
            -- Create new bed record if it doesn't exist
            INSERT INTO beds (bed_number, room_type, daily_rate, status, ward_name)
            VALUES (
                admission_record.bed_number,
                UPPER(admission_record.room_type),
                admission_record.daily_rate,
                'OCCUPIED',
                'Ward ' || admission_record.bed_number
            ) RETURNING id INTO bed_record;
