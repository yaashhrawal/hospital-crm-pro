-- SIMPLE PATIENT DELETION SCRIPT
-- Run each section one by one in Supabase SQL Editor

-- 🔍 STEP 1: First, let's see which patients exist
SELECT patient_id, first_name, last_name, created_at 
FROM patients 
WHERE patient_id IN (
  'P000046', 'P000045', 'P000001', 'P000043', 'P000042', 'P000041', 'P000040', 'P000039', 'P000038', 'P000037',
  'P000036', 'P000035', 'P000034', 'P000033', 'P000032', 'P000031', 'P000030', 'P000029', 'P000028', 'P000027',
  'P000026', 'P000025', 'P000024', 'P000023', 'P000022', 'P000021', 'P000020', 'P000019', 'P000018', 'P000017',
  'P000016', 'P000015', 'P000014', 'P000013', 'P000012', 'P000011', 'P000010', 'P000009', 'P000008', 'P000007',
  'P000006', 'P0003', 'P0005', 'P0002', 'P0001', 'P0004'
)
ORDER BY patient_id;

-- 🗑️ STEP 2: Delete patient transactions first
DELETE FROM patient_transactions 
WHERE patient_id IN (
  SELECT id FROM patients 
  WHERE patient_id IN (
    'P000046', 'P000045', 'P000001', 'P000043', 'P000042', 'P000041', 'P000040', 'P000039', 'P000038', 'P000037',
    'P000036', 'P000035', 'P000034', 'P000033', 'P000032', 'P000031', 'P000030', 'P000029', 'P000028', 'P000027',
    'P000026', 'P000025', 'P000024', 'P000023', 'P000022', 'P000021', 'P000020', 'P000019', 'P000018', 'P000017',
    'P000016', 'P000015', 'P000014', 'P000013', 'P000012', 'P000011', 'P000010', 'P000009', 'P000008', 'P000007',
    'P000006', 'P0003', 'P0005', 'P0002', 'P0001', 'P0004'
  )
);

-- 🗑️ STEP 3: Delete patient admissions
DELETE FROM patient_admissions 
WHERE patient_id IN (
  SELECT id FROM patients 
  WHERE patient_id IN (
    'P000046', 'P000045', 'P000001', 'P000043', 'P000042', 'P000041', 'P000040', 'P000039', 'P000038', 'P000037',
    'P000036', 'P000035', 'P000034', 'P000033', 'P000032', 'P000031', 'P000030', 'P000029', 'P000028', 'P000027',
    'P000026', 'P000025', 'P000024', 'P000023', 'P000022', 'P000021', 'P000020', 'P000019', 'P000018', 'P000017',
    'P000016', 'P000015', 'P000014', 'P000013', 'P000012', 'P000011', 'P000010', 'P000009', 'P000008', 'P000007',
    'P000006', 'P0003', 'P0005', 'P0002', 'P0001', 'P0004'
  )
);

-- 🗑️ STEP 4: Delete future appointments
DELETE FROM future_appointments 
WHERE patient_id IN (
  SELECT id FROM patients 
  WHERE patient_id IN (
    'P000046', 'P000045', 'P000001', 'P000043', 'P000042', 'P000041', 'P000040', 'P000039', 'P000038', 'P000037',
    'P000036', 'P000035', 'P000034', 'P000033', 'P000032', 'P000031', 'P000030', 'P000029', 'P000028', 'P000027',
    'P000026', 'P000025', 'P000024', 'P000023', 'P000022', 'P000021', 'P000020', 'P000019', 'P000018', 'P000017',
    'P000016', 'P000015', 'P000014', 'P000013', 'P000012', 'P000011', 'P000010', 'P000009', 'P000008', 'P000007',
    'P000006', 'P0003', 'P0005', 'P0002', 'P0001', 'P0004'
  )
);

-- 🗑️ STEP 5: Finally, delete the patients
DELETE FROM patients 
WHERE patient_id IN (
  'P000046', 'P000045', 'P000001', 'P000043', 'P000042', 'P000041', 'P000040', 'P000039', 'P000038', 'P000037',
  'P000036', 'P000035', 'P000034', 'P000033', 'P000032', 'P000031', 'P000030', 'P000029', 'P000028', 'P000027',
  'P000026', 'P000025', 'P000024', 'P000023', 'P000022', 'P000021', 'P000020', 'P000019', 'P000018', 'P000017',
  'P000016', 'P000015', 'P000014', 'P000013', 'P000012', 'P000011', 'P000010', 'P000009', 'P000008', 'P000007',
  'P000006', 'P0003', 'P0005', 'P0002', 'P0001', 'P0004'
);

-- ✅ STEP 6: Verify deletion (should return 0 rows)
SELECT patient_id, first_name, last_name 
FROM patients 
WHERE patient_id IN (
  'P000046', 'P000045', 'P000001', 'P000043', 'P000042', 'P000041', 'P000040', 'P000039', 'P000038', 'P000037',
  'P000036', 'P000035', 'P000034', 'P000033', 'P000032', 'P000031', 'P000030', 'P000029', 'P000028', 'P000027',
  'P000026', 'P000025', 'P000024', 'P000023', 'P000022', 'P000021', 'P000020', 'P000019', 'P000018', 'P000017',
  'P000016', 'P000015', 'P000014', 'P000013', 'P000012', 'P000011', 'P000010', 'P000009', 'P000008', 'P000007',
  'P000006', 'P0003', 'P0005', 'P0002', 'P0001', 'P0004'
);